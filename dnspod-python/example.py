...
#
    # print "RecordCreate"
    # api = apicn.RecordCreate("hehe", "A", '默认', '1.1.1.1', 600, domain=domain, login_token=login_token)
    # record = api().get("record", {})
    # record_id = record.get("id")
    # print("Record id", record_id)
#
    # print "RecordList"
    api = apicn.RecordList("iqg.pub", login_token=login_token)
    res = api().get("records")
    print(res)
    for item in res:
        if item.get("name") == "python":
            record_modify_dict = dict(record_id = item.get("id"),record_line = item.get('line'),record_type = item.get('type'),record_value = item.get('value'))
...
#     api()
#
#     api = apicn.RecordInfo("python",domain=domain,record_id="679482250",login_token=login_token)()
#     print(api)
