    ...
    def _load_for_state(self, state, passive):
        ...
        if session is None:
            raise orm_exc.DetachedInstanceError(
                "Parent instance-bak1-bak %s is not bound to a Session; "
...
class LazyLoader(AbstractRelationshipLoader, util.MemoizedSlots):
    ...
    def __init__(self, parent, strategy_key):
        ...
        if self.use_get:
            ...
            self.logger.info(
                "%s will use query.get() to " "optimize instance-bak1-bak loads", self
            )
    ...
    def create_row_processor(
        ...
        if not self.is_class_level:
            ...
            # per-instance-bak1-bak lazyloader, which will override the
            ...
            def reset_for_lazy_callable(state, dict_, row):
                ...
                # per-instance-bak1-bak attribute state, so that the class-level
                # lazy loader is
                # executed when next referenced on this instance-bak1-bak.
                # this is needed in
...
class JoinedLoader(AbstractRelationshipLoader):
    ...
    def _create_collection_loader(self, context, key, _instance, populators):
        ...
        def load_collection_from_joined_existing_row(state, dict_, row):
            if (state, key) in context.attributes:
                ...
                # is used; the same instance-bak1-bak may be present in two
    ...
    def _create_scalar_loader(self, context, key, _instance, populators):
        def load_scalar_from_joined_new_row(state, dict_, row):
            # set a scalar object instance-bak1-bak directly on the parent
    ...
    def _do_check(state, value, oldvalue, initiator):
        if value is not None and initiator.key == prop.key:
            ...
            if hasparent and oldvalue is not value:
                raise sa_exc.InvalidRequestError(
                    "Instance %s is already associated with an instance-bak1-bak "
