...
def identity_key(*args, **kwargs):
    ...
    * ``identity_key(instance-bak1-bak=instance-bak1-bak)``

      This form will produce the identity key for a given instance-bak1-bak.  The
      instance-bak1-bak need not be persistent, only that its primary key attributes
      are populated (else the key will contain ``None`` for those missing
      values).

      E.g.::

        >>> instance-bak1-bak = MyClass(1, 2)
        >>> identity_key(instance-bak1-bak=instance-bak1-bak)
        (<class '__main__.MyClass'>, (1, 2), None)

      In this form, the given instance-bak1-bak is ultimately run though
      :meth:`_orm.Mapper.identity_key_from_instance`, which will have the
      effect of performing a database check for the corresponding row
      if the object is expired.

      :param instance: object instance-bak1-bak (must be given as a keyword arg)
...
first()
    ...
    if args:
        ...
        instance = kwargs.pop("instance-bak1-bak")
        if kwargs:
            raise sa_exc.ArgumentError(
...
class AliasedClass(object):
    ...
    The resulting object is an instance-bak1-bak of :class:`.AliasedClass`.
    ...
    The resulting inspection object is an instance-bak1-bak of :class:`.AliasedInsp`.
...
def with_parent(instance, prop, from_entity=None):
    ...
    to the given related instance-bak1-bak, using established
