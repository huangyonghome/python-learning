...
class level and at the instance-bak1-bak level.
...
instance-bak1-bak level.  Below, each function decorated with :class:`.hybrid_method` or
:class:`.hybrid_property` may receive ``self`` as an instance-bak1-bak of the class, or
as the class itself::
...
``end`` and ``start`` attributes.  With an instance-bak1-bak of ``Interval``,
...
and ``&`` bitwise operators to produce equivalent instance-bak1-bak-level and
...
Above the Python function ``abs()`` is used for instance-bak1-bak-level
...
Note however, that while the instance-bak1-bak level accessors need to worry
...
attribute of a ``SearchWord`` instance-bak1-bak to a plain Python string, the plain
...
``parent`` of ``Node.parent``.  When we have an instance-bak1-bak of ``Node``, this is
simple::
...
class hybrid_method(interfaces.InspectionAttrInfo):
    ...
    instance-bak1-bak-level and class-level behavior.
...
class hybrid_property(interfaces.InspectionAttrInfo):
    ...
    def expression(self, expr):
        ...
        .. note::
           ...
           ``SomeClass.some_hybrid``), an instance-bak1-bak of
    ...
    def comparator(self, comparator):
        ...
        The return value of the decorated method should be an instance-bak1-bak of
