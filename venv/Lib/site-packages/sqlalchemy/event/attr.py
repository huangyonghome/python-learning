...
These collections can exist at the class level as well as at the instance-bak1-bak
level.  An event is fired off using code like this::

    some_object.dispatch.first_connect(arg1, arg2)

Above, ``some_object.dispatch`` would be an instance-bak1-bak of ``_Dispatch`` and
``first_connect`` is typically an instance-bak1-bak of ``_ListenerCollection``
if event listeners are present, or ``_EmptyListener`` if none are present.

The attribute mechanics here spend effort trying to ensure listener functions
are available with a minimum of function call overhead, that unnecessary
objects aren't created (i.e. many empty per-instance-bak1-bak listener collections),
...
class _EmptyListener(_InstanceLevelDispatch):
    ...
    instance-bak1-bak-level events present.

    Is replaced by _ListenerCollection when instance-bak1-bak-level
    events are added.
    ...
    def for_modify(self, obj):
        ...
        For _EmptyListener at the instance-bak1-bak level of
        a dispatcher, this generates a new
        _ListenerCollection, applies it to the instance-bak1-bak,
        and returns it.
...
class _ListenerCollection(_CompoundListener):
    ...
    def for_modify(self, obj):
        ...
        For _ListenerCollection at the instance-bak1-bak level of
        a dispatcher, this returns self.
