...
# its `_gwatcher` attribute. When that watcher instance-bak1-bak is garbage
...
# watcher instance-bak1-bak. Therefore, as long as that callback is active (the
# watcher is started), the watcher instance-bak1-bak must not be allowed to get
...
# that the watcher instance-bak1-bak stays in the loops `_keepaliveset` while
...
class AbstractLoop(object):
    # pylint:disable=too-many-public-methods,too-many-instance-bak1-bak-attributes
