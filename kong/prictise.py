    ...
    if plugins:
        ...
        for key in plugins:
            ...
            if not route_name and not service_name:
                raise AttributeError("plugin插件:{},必须指定配置在某个Service或者routes下".format(plugins_name))
            ...
            #如果有配置routes路由名,则关联到routes路由中
                ...
                if not route_name_id:
                    raise NameError("plugins插件:{},配置的route路由:{}不存在!".format(plugins_name,route_name))
                else:
                    #拼接插件的url.
                    ...
                    if not res:
                        print("plugins:{}创建成功.绑定在route路由:{}中".format(plugins_name,route_name))
                    else:
            ...
            #如果只配置了service名称,则将插件配置到service下
                ...
                service_id = get_service(service_name)
                if not service_id:
                    raise NameError("plugins插件:{},配置的Service:{}不存在!".format(plugins_name,service_name))
                else:
                    ...
                    res = post_plugins(plugins_url, plugins_data)
                    if not res:
                        print("plugins:{}创建成功.绑定在service:{}中".format(plugins_name,service_name))
                    else:
                        print("plugins:{}创建失败:{}".format(plugins_name, res))
